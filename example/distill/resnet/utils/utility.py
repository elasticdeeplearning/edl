...
def create_data_loader(is_train, args, data_layout='NCHW'):
    ...
    if is_train and args.use_mixup:
        ...
        teacher_predict = fluid.data(
            name='teacher_predict',
            shape=[None, args.class_dim],
            dtype='float32')

        data_loader = fluid.io.DataLoader.from_generator(
            feed_list=[feed_image, feed_label, teacher_predict],
            capacity=64,
            use_double_buffer=True,
            iterable=True)
        return data_loader, [feed_image, feed_label, teacher_predict]
    else:
        if args.use_dali:
