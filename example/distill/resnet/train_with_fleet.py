...
add_arg('lr_strategy',      str,   "cosine_warmup_decay",    "Set the learning rate decay strategy.")
...
add_arg('use_distill_service',     bool,  False,     "Whether to use distill service train")
add_arg('distill_teachers',         str,  None,  "distill fixed teacher")
...
def net_config(image, model, args, is_train, label=0, y_a=0, y_b=0, lam=0.0, teacher_label=None, data_format="NCHW"):
    ...
    if not args.is_distill:
        ...
        if is_train:
            if use_mixup:
                ...
                return avg_cost
            elif args.use_distill_service:
                teacher_smooth_out = fluid.layers.label_smooth(label=teacher_label,
                                                               epsilon=0.0,
                                                               dtype="float32")
                cost, softmax_out = fluid.layers.softmax_with_cross_entropy(
                    out, teacher_smooth_out, soft_label=True, return_softmax=True)
            else:
...
def build_program(is_train, main_prog, startup_prog, args, dist_strategy=None, data_layout="NCHW"):
    ...
    with fluid.program_guard(main_prog, startup_prog):
        ...
        with fluid.unique_name.guard():
            ...
            elif is_train and args.use_distill_service:
                image, label, teacher_label = data[0], data[1], data[2]
                avg_cost, acc_top1, acc_top5 = net_config(image, model, args, label=label, is_train=is_train,
                                                          teacher_label=teacher_label, data_format=data_layout)
...
def train(args):
    ...
    if args.use_dali:
        ...
        if args.use_distill_service:
            ...
            if args.distill_teachers is not None:
