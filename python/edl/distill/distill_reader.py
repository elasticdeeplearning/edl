...
class DynamicServiceDiscover(ServiceDiscover):
    ...
    def _connect(self):
        if self._type == "etcd":
            from edl.distill.discovery_client import DiscoveryClient
        elif self._type == "redis":
            from edl.distill.redis.client import Client as DiscoveryClient
        else:
            assert False, "BALANCE_TYPE must be etcd or redis"
        client = DiscoveryClient(
            self._discovery_servers, self._service_name, self._require_num
        )
...
class DistillReader(object):
            ...
            while _service_discover is None:
                if self._mode == "fixed":
                    ...
                    _service_discover = DynamicServiceDiscover(
                        self._discovery_servers, self._require_num, self._service_name
                    )
                else:
    ...
    def _init_args(self):
        if not self._is_args_init:
            ...
            self._predict_stop_events = [mps.Event() for i in range(self._require_num)]
    ...
    def _init_conf_file_from_env(self):
        ...
        assert conf_file is not None, (
            ...
            "of conf_file. In the future, we will cancel the settings of this file".format(
                self._serving_conf_file
            )
        )
    ...
    def set_dynamic_teacher(
        self, discovery_servers, teacher_service_name, require_max_teacher=1
    ):
    ...
    def __del__(self):
        ...
        for i in range(20):
            if self._reader_worker.is_alive() or self._predict_manage_thread.is_alive():
                time.sleep(1)
