...
#clean keys
rank_key="/test_success_job/pod_rank/nodes"
etcdctl del ${rank_key}/0 ${rank_key}/1
flag_key="/job_id/job_flag/nodes/complete"
etcdctl del ${flag_key}
...
timeout -s SIGKILL ${run_time} python -m edl.collective.launch launch_demo.py > ${name}_run_00.log 2>&1 &
pid_00=$!

export CUDA_VISIBLE_DEVICES=1
export PADDLE_DEMO_EXIT_CODE=0
timeout -s SIGKILL ${run_time} python -m edl.collective.launch launch_demo.py > ${name}_run_01.log 2>&1 &
pid_01=$!
